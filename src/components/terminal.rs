...
use crate::components::history::{HistoryItem, create_welcome_entry, create_command_entry, create_html_entry};
use crate::components::syntax::{parse_syntax_segments, render_syntax_segments};
use crate::filesystem::fetch_file_content;
            ...
            match e.key().as_str() {
                "Enter" => {
                    ...
                    if !input_value.trim().is_empty() {
                        ...
                        // Format the output based on execution result
                        let (output, is_error) = match result {
                            CommandResult::Success(output) => (output, false),
                            CommandResult::Error(error) => (format!("Error: {}", error), true),
                        };

                        // Check for special command types
                        if output == "__CLEAR_SCREEN__" {
                            // Clear the screen by resetting history to just the welcome message
                            let welcome_history = vec![create_welcome_entry()];
                            history.set(welcome_history);
                        } else if output.starts_with("__FETCH_FILE__:") {
                            // Handle file fetching for cat command
                            let file_path = &output[15..]; // Remove "__FETCH_FILE__:"
                            let history_clone = history.clone();
                            let command_clone = command.clone();
                            let file_path_clone = file_path.to_string();
                            
                            wasm_bindgen_futures::spawn_local(async move {
                                match fetch_file_content(&file_path_clone).await {
                                    Ok(content) => {
                                        let mut new_history = (*history_clone).clone();
                                        new_history.push(create_command_entry(command_clone, content, false));
                                        history_clone.set(new_history);
                                    }
                                    Err(error) => {
                                        let mut new_history = (*history_clone).clone();
                                        new_history.push(create_command_entry(command_clone, format!("Error: {}", error), true));
                                        history_clone.set(new_history);
                                    }
                                }
                            });
                        } else if output.starts_with("__FETCH_MARKDOWN__:") {
                            // Handle markdown file fetching for view command
                            let file_path = &output[19..]; // Remove "__FETCH_MARKDOWN__:"
                            let history_clone = history.clone();
                            let command_clone = command.clone();
                            let file_path_clone = file_path.to_string();
                            ...
                            wasm_bindgen_futures::spawn_local(async move {
                                match fetch_file_content(&file_path_clone).await {
                                    Ok(content) => {
                                        // Simple markdown to HTML conversion (you can enhance this later)
                                        let html_content = markdown_to_html(&content);
                                        let mut new_history = (*history_clone).clone();
                                        new_history.push(create_html_entry(command_clone, html_content));
                                        history_clone.set(new_history);
                                    }
                                    Err(error) => {
                                        let mut new_history = (*history_clone).clone();
                            ...
                            if output.starts_with("__HTML_CONTENT__:") {
                                let html_content = &output[17..]; // Remove "__HTML_CONTENT__:"
                                new_history.push(create_html_entry(command.clone(), html_content.to_string()));
                            } else {
                                new_history.push(create_command_entry(command.clone(), output, is_error));
                            }
